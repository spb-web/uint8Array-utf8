...
const transformChar = (char: number, excludeCharacters: Set<number>) => excludeCharacters.has(char) ? (0x8000 | char) : char
...
export const uint8ArrayToUtf8 = (bytes: Uint8Array, excludeCharacters = new Set<number>()): string => {
  ...
  for(let byteIndex = 0; byteIndex < dataSize; byteIndex++) {
    ...
    residueIndex += 1
    chars[charIndex] = transformChar(residue | (byte >>> residueIndex), excludeCharacters)
    charIndex += 1
    residue = (byte & ((2 ** residueIndex) - 1)) << (UTF8_SIGNIFICANT_BITS_SIZE - residueIndex) 
     
    if (residueIndex >= UTF8_SIGNIFICANT_BITS_SIZE) {
      chars[charIndex] = transformChar(residue, excludeCharacters)
      charIndex += 1
      residueIndex = 0
      residue = 0
    }
